  ...
  class SpdxService < BaseService
    class << self
      ...
      def landing_page_url
        Rails.configuration.x.dmproadmap.spdx_landing_page_url || super
      end

      def api_base_url
        Rails.configuration.x.dmproadmap.spdx_api_base_url || super
      end

      def max_pages
        Rails.configuration.x.dmproadmap.spdx_max_pages || super
      end

      def max_results_per_page
        Rails.configuration.x.dmproadmap.spdx_max_results_per_page || super
      end

      def max_redirects
        Rails.configuration.x.dmproadmap.spdx_max_redirects || super
      end

      def active?
        Rails.configuration.x.dmproadmap.spdx_active || super
      end

      def list_path
        Rails.configuration.x.dmproadmap.spdx_list_path
      end
