  ...
  class OrcidService < BaseDmpIdService
    class << self
      ...
      def xml_for(plan:, dmp_id:, user:)
        return nil unless plan.is_a?(Plan) && dmp_id.present? && user.is_a?(User)
          ...
          <work:work xmlns:common="http://www.orcid.org/ns/common"
            ...
            <common:external-ids>
              <common:external-id>
                ...
                <common:external-id-value>#{dmp_id.gsub('https://doi.org/', '')}</common:external-id-value>
                <common:external-id-url>#{dmp_id}</common:external-id-url>
