  ...
  module V1
    ...
    class ResearchOutputPresenter
      ...
      def load_narrative_content
        ...
        if Rails.configuration.x.dmproadmap.madmp.extract_preservation_statements_from_themed_questions
          @preservation_statement = fetch_q_and_a_as_single_statement(themes: %w[Preservation])
        end
        if Rails.configuration.x.dmproadmap.madmp.extract_security_privacy_statements_from_themed_questions
          @security_and_privacy = fetch_q_and_a(themes: ['Ethics & privacy', 'Storage & security'])
        end
        if Rails.configuration.x.dmproadmap.madmp.extract_data_quality_statements_from_themed_questions
