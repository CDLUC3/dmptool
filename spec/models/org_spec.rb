...
RSpec.describe Org do
  ...
  context 'scopes' do
    ...
    describe '.default_orgs' do
      ...
      context 'when Org has same abbr as dmproadmap.rb initializer setting' do
        let!(:org) do
          abbrev = Rails.configuration.x.dmproadmap.organisation_abbreviation
  ...
  describe '#org_admin_plans' do
    before do
      @original_reg = Rails.configuration.x.dmproadmap.plans_org_admins_read_all
    end

    after do
      Rails.configuration.x.dmproadmap.plans_org_admins_read_all = @original_reg
    end
